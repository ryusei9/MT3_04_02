...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	// ボール
	Ball ball{};
	ball.position = { 0.51f,0.38f,0.0f };
	ball.mass = 2.0f;
	ball.radius = 10.0f;
	// 角速度
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::Checkbox("Start", &isSwingStart);
		...
		DrawPendulum(pendulum,ball, worldViewProjectionMatrix, viewPortMatrix);
		///
		/// ↑描画処理ここまで
		///
...
void Hook(Spring& s, Ball& b) {
	...
	if (length != 0.0f) {
		...
		b.aceleration = force / b.mass;
	}
	// 加速度も速度どちらも秒を基準とした値である
	// それが、1/60秒間(deltaTime)適用されたと考える
	b.velocity = b.velocity + b.aceleration * deltaTime;
